  ...
  try {
    ...
    if (importModule === "booking") {
      ...
      const rows = payload.rows.map((row, index) => {
        ...
        return { id: crypto.randomUUID(), ...common, asOfDate: String(row.as_of_date), branch: textValue(row.branch ?? row.branch_name ?? row.branch_code), product: textValue(row.product ?? row.product_type ?? row.product_model), quantity, stockDate, branchCode: nullableText(row.branch_code), branchName: nullableText(row.branch_name), productType: nullableText(row.product_type), productGroup: nullableText(row.product_group), productModel: nullableText(row.product_model ?? row.model), kmmFlag: integerValue(row.kmm_flag), msrp: nullableText(row.msrp), stockStatus: nullableText(row.stock_status ?? row.status), stockNumber: nullableText(row.stock_number), serialNumber: nullableText(row.serial_number), engineNumber: nullableText(row.engine_number), chassisNumber: nullableText(row.chassis_number), stockAgeDays, snapshotDate };
      });
