...
export function TrendChartToolbar({ metric, selectedYears, period, onMetricChange, onYearChange, onPeriodChange, metricOptions = defaultMetrics, yearOptions = defaultYears }: TrendChartToolbarProps) {
  ...
  return <div className="flex w-full shrink-0 flex-wrap items-center justify-end gap-3 whitespace-nowrap sm:w-auto">
    <select aria-label="Metric" value={metric} onChange={(event) => onMetricChange(event.target.value)} className="h-11 min-w-[124px] rounded-[var(--radius-control)] border border-[var(--border-default)] bg-[var(--surface-default)] px-3 text-sm font-medium text-[var(--text-primary)] shadow-[var(--shadow-card)] outline-none transition hover:bg-[var(--surface-subtle)] focus:border-[var(--focus-ring)] focus:ring-2 focus:ring-[var(--brand-focus)]">
      {metricOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}
    </select>
    <div ref={rootRef} className="relative">
      <button type="button" onClick={() => setOpen((value) => !value)} aria-label="Compare Year" aria-expanded={open} className="flex h-11 min-w-[132px] items-center justify-between gap-2 rounded-[var(--radius-control)] border border-[var(--border-default)] bg-[var(--surface-default)] px-3 text-sm font-medium text-[var(--text-primary)] shadow-[var(--shadow-card)] transition hover:bg-[var(--surface-subtle)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)] focus-visible:ring-offset-2">
      ...
      {open && <div className="absolute right-0 top-[48px] z-40 w-44 rounded-[var(--radius-control)] border border-[var(--border-default)] bg-[var(--surface-elevated)] p-1.5 shadow-[var(--shadow-floating)]">
    ...
    <select aria-label="Period" value={period} onChange={(event) => onPeriodChange(event.target.value as TrendPeriod)} className="h-11 min-w-[112px] rounded-[var(--radius-control)] border border-[var(--border-default)] bg-[var(--surface-default)] px-3 text-sm font-medium text-[var(--text-primary)] shadow-[var(--shadow-card)] outline-none transition hover:bg-[var(--surface-subtle)] focus:border-[var(--focus-ring)] focus:ring-2 focus:ring-[var(--brand-focus)]">
