...
export function StandardLineChart({ title, subtitle, labels, series, unit = "Unit", height = 420, formatValue = (value) => value.toLocaleString(), metricOptions, defaultMetric, onMetricChange, defaultSeriesIds, defaultPeriod = "ytd", onPeriodChange, className = "" }: StandardLineChartProps) {
  ...
  return <section className={`rounded-[var(--radius-card)] border border-[var(--border-default)] bg-[var(--surface-default)] p-5 shadow-[var(--shadow-card)] sm:p-6 ${className}`}>
    {title && <header className="flex flex-col items-start justify-between gap-3 border-b border-[var(--divider)] pb-4 sm:flex-row sm:items-center sm:gap-6">
      <div className="shrink-0"><h2 className="text-[20px] font-semibold tracking-normal text-[var(--text-primary)]">{title}</h2>{subtitle && <p className="mt-0.5 text-xs text-[var(--text-secondary)]">{subtitle}</p>}</div>
    ...
    <div className="mt-4 flex flex-wrap items-center gap-x-6 gap-y-2 text-xs font-semibold text-[var(--text-secondary)]">{shown.map((item, index) => { const token = style(item, index); return <span key={item.id} className="flex items-center gap-2"><i className="h-0 w-7 border-t-[3px]" style={{ borderColor: token.color, borderStyle: token.dash ? "dashed" : "solid" }} />{item.kind === "target" ? "Target" : item.year ?? item.label}</span>; })}</div>
