...
const panelClass = "min-w-0 overflow-hidden border-[var(--border-default)] bg-[var(--surface-default)] shadow-[var(--shadow-card)]";
...
function downloadReport(input: DailyManagementInputSnapshot, snapshot: DailyManagementSnapshot | null, companyCode: string) {
  const rows = [
    [`${companyCode} DAILY MANAGEMENT REPORT`],
  ...
  link.href = url;
  link.download = `${companyCode.toLowerCase()}-daily-management-${input.reportDate}.csv`;
  link.click();
  URL.revokeObjectURL(url);
}

function inputEditHref(input: DailyManagementInputSnapshot | null) {
  if (!input) return "/daily-management/input";
  const search = new URLSearchParams({ date: input.reportDate, branch: input.branch });
  return `/daily-management/input?${search}`;
}
  ...
  return (
    <Card className={panelClass}>
      ...
      <div className="grid gap-3 p-3 lg:grid-cols-[minmax(0,1fr)_190px]">
        <div className="min-w-0 rounded-[14px] border border-[var(--border-subtle)] bg-white p-3" role="img" aria-label="Branch sales mix relative to the highest-selling branch">
          ...
          <div className="space-y-3">{branches.map((row) => <div key={row.branch} className="grid grid-cols-[minmax(110px,0.85fr)_minmax(110px,1.4fr)_50px] items-center gap-3"><span className="min-w-0 text-[9px] font-semibold leading-3">{row.branch}</span><div className="relative h-3 overflow-hidden rounded-full bg-[var(--surface-muted)]"><div className="h-full rounded-full bg-[var(--brand-500)]" style={{ width: `${Math.max(4, (row.sales / maxBranchSales) * 100)}%` }} /></div><span className="kmm-tabular text-right text-[9px]"><strong>{row.sales}</strong><span className="text-[var(--text-tertiary)]"> units</span></span></div>)}</div>
        ...
        <div className="min-w-0 rounded-[14px] bg-[var(--surface-subtle)] p-3"><PerformerList title="Top Performers" rows={topPerformers} /></div>
      </div>
    </Card>
  );
}
        ...
        <div className="space-y-2" role="img" aria-label="Booking pipeline distribution">
          {stages.map((stage) => (
            <div key={stage.label} className="grid grid-cols-[82px_minmax(0,1fr)_28px] items-center gap-2">
              ...
              <div className="h-5 overflow-hidden rounded-full bg-[var(--surface-muted)]"><div className="flex h-full min-w-5 items-center justify-end rounded-full pr-1.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.35)]" style={{ width: `${Math.max(7, (stage.value / maxValue) * 100)}%`, background: `linear-gradient(90deg, color-mix(in srgb, ${stage.color} 68%, white), ${stage.color})` }}><span className="text-[8px] font-bold text-white">{stage.value}</span></div></div>
...
function StockHealth({ snapshot, currency }: { snapshot: DailyManagementSnapshot; currency: string }) {
...
export function DailyManagementPage() {
  ...
  const [timeZone, setTimeZone] = useState(selectedCompany?.timeZone ?? "Asia/Yangon");
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      setSnapshot(null);
      try {
        const remote = await loadDailyManagementInput("published", { companyId });
        if (!active) return;
        if (!remote) {
          setLoadState("empty");
          return;
        }
        const live = await loadDailyManagementReport({ date: remote.reportDate, branch: remote.branch === "All Branches" ? undefined : remote.branch, companyId });
        if (active) {
          setInput(remote);
    ...
    <div className="min-h-[calc(100vh-72px)] bg-[var(--surface-canvas)] text-[var(--text-primary)]">
      <main className="mx-auto max-w-[1900px] p-3 sm:p-4 xl:px-5 xl:py-4">
        <div className="space-y-3">
          <PageHeader
            eyebrow={ready ? "Live · D1 Data" : loadState === "error" ? "D1 unavailable" : loadState === "empty" ? "No published report" : "Loading D1 data"}
            title={t("route.dailyManagement.title")}
            description={reportError ? `D1 unavailable: ${reportError}` : t("route.dailyManagement.subtitle").replaceAll("KMM", companyCode)}
            action={<div className="flex flex-wrap items-center gap-2 text-[11px]">{input && <><span className="rounded-[var(--radius-control)] border border-[var(--border-default)] bg-[var(--surface-default)] px-3 py-2"><CalendarDays size={12} className="mr-1.5 inline" />{input.reportDate}</span><span className="rounded-[var(--radius-control)] border border-[var(--border-default)] bg-[var(--surface-default)] px-3 py-2">Updated {publishedTime ?? "unavailable"} · {timeZone}</span></>}<a href={inputEditHref(input)} className="inline-flex min-h-11 items-center gap-2 rounded-[var(--radius-control)] border border-[var(--border-default)] bg-[var(--surface-default)] px-3 font-semibold text-[var(--text-secondary)] transition-colors hover:bg-[var(--surface-subtle)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)]"><FilePenLine size={14} />Update Inputs</a>{ready && <ExportButton onClick={() => downloadReport(input, snapshot, companyCode)} />}</div>}
          />
          {!ready ? <Card className={`${panelClass} grid min-h-64 place-items-center p-8 text-center`} role={loadState === "error" ? "alert" : "status"}><div className="max-w-md"><strong className="text-sm">{loadState === "loading" ? "Loading authenticated D1 data…" : loadState === "empty" ? "No published Daily Management input" : "Daily Management data could not be loaded"}</strong><p className="mt-2 text-[11px] leading-5 text-[var(--text-secondary)]">{loadState === "empty" ? "Publish a Daily Management input before opening the report. No sample values are shown." : loadState === "error" ? reportError : "The report remains blank until the live response is complete."}</p>{loadState === "error" && <button type="button" onClick={() => setRefreshKey((value) => value + 1)} className="mt-4 min-h-10 rounded-[10px] bg-[var(--brand-500)] px-4 text-[11px] font-semibold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)]">Retry</button>}</div></Card> : <>
            ...
            <div className="grid gap-3 xl:grid-cols-[1.08fr_0.92fr]"><TodayDetail snapshot={snapshot} /><StockHealth snapshot={snapshot} currency={currency} /></div>
            ...
            <footer className="flex flex-col gap-1 border-t border-[var(--divider)] pt-2 text-[8px] text-[var(--text-tertiary)] sm:flex-row sm:justify-between"><span>MTD = Month To Date</span><span>Operational data: authenticated D1 Sales, Booking and Stock</span><span className="flex items-center gap-1"><PackageCheck size={10} />{companyCode} Sales Division</span></footer>
          </>}
        </div>
      </main>
    </div>
