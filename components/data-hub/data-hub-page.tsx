...
function LegacyDataHubPage() {
  ...
  async function saveCurrentMapping() { if (!mappable) return; saveMapping(selectedModule, columnMappings); try { await persistSalesMapping(Object.fromEntries(columnMappings.filter((item) => item.excelColumn !== "—").map((item) => [item.excelColumn, item.fieldKey]))); setMappingSaved(true); } catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to save this mapping."); } }
