...
export function MarketingIntelligencePage() {
    ...
    } }); periodSales.filter((row) => product === "All" || productGroup(row.productType) === product).forEach((row) => { const resolved = resolveTownship(row.township); if (!resolved.key)
    ...
    } }); marketing.forEach((row) => { const resolved = resolveTownship(row.township); if (!resolved.key)
