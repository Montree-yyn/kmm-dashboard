...
function Trend({ rows }: {
    rows: MarketingRow[];
}) { const [tooltip, setTooltip] = useState<{ label: string; value: number | null; x: number; y: number } | null>(null); const width = 760; const height = 320; const padX = 42; const padTop = 26; const padBottom = 42; const latestMonth = Math.max(...rows.map((row) => row.month ?? 0), 0); const values = MONTHS.map((_, index) => { const month = index + 1; if (latestMonth > 0 && month > latestMonth) return null; return rows.filter((row) => row.month === month).length; }); const plottedValues = values.filter((value): value is number => value !== null); const max = Math.max(...plottedValues, 1); const xFor = (index: number) => padX + (index * (width - padX * 2)) / 11; const yFor = (value: number) => height - padBottom - (value / max) * (height - padTop - padBottom); const path = values.reduce((result, value, index) => { if (value === null) return result; const x = xFor(index); const y = yFor(value); const previous = values[index - 1]; if (previous === null || previous === undefined) return `${result} M ${x} ${y}`; const previousX = xFor(index - 1); const previousY = yFor(previous); const middle = (previousX + x) / 2; return `${result} C ${middle} ${previousY}, ${middle} ${y}, ${x} ${y}`; }, ""); return <div className="relative -mt-2"><svg className="h-[280px] w-full md:h-[320px]" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" role="img" onMouseLeave={() => setTooltip(null)}><title>Monthly activity trend</title>{[0, 0.25, 0.5, 0.75, 1].map((ratio) => { const y = height - padBottom - ratio * (height - padTop - padBottom); return <line key={ratio} x1={padX} x2={width - padX} y1={y} y2={y} stroke="#F3F4F6"/>; })}<path d={path} fill="none" stroke="#FF7A00" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>{values.map((value, index) => value === null ? null : <circle key={MONTHS[index]} cx={xFor(index)} cy={yFor(value)} r={tooltip?.label === MONTHS[index] ? 5 : 3.8} fill="white" stroke="#FF7A00" strokeWidth="2.4" onMouseEnter={() => setTooltip({ label: MONTHS[index], value, x: (xFor(index) / width) * 100, y: yFor(value) })}/>) }{MONTHS.map((month, index) => <text key={month} x={xFor(index)} y={height - 11} textAnchor="middle" fontSize="11" fontWeight="600" fill="#6B7280">{month}</text>)}</svg>{tooltip && <div className="pointer-events-none absolute z-10 rounded-lg border border-[#E5E7EB] bg-white px-3 py-2 text-xs text-[#4B5563] shadow-[0_10px_28px_rgba(31,41,55,0.10)]" style={{ left: `${Math.min(Math.max(tooltip.x, 8), 78)}%`, top: `${Math.max(tooltip.y - 36, 4)}px` }}><p className="font-bold text-[#1F2937]">{tooltip.label}</p><p className="mt-1">Activities: <strong>{tooltip.value === null ? "No data" : count(tooltip.value)}</strong></p></div>}</div>; }
