...
import { Bell, ChevronDown, Maximize2, Menu, RefreshCw, RotateCcw, Search, X } from "lucide-react";
...
function Phase1TownshipPanel({ metric, activeMetric, filterContext, onClose, prior = metric?.priorSales ?? null, yoyState = metric?.yoyState ?? "no-prior-data", benchmark = metric?.benchmark ?? null }: { metric: TownshipMetric | null; activeMetric: ExecutiveGisFilters["activeMetric"]; filterContext: { year: string; month: string; product: string }; onClose?: () => void; prior?: SalesTotals | null; yoyState?: YoYState; benchmark?: TownshipBenchmark | null }) {
    ...
    const primaryRows = current ? [{ key: "salesUnit" as const, label: "Unit", value: metric?.hasFilteredSalesData ? count(current.salesUnit) : "รอข้อมูล" }, { key: "salesValue" as const, label: "Value", value: metric?.hasFilteredSalesData ? `${compact(current.salesValue)} MMK` : "รอข้อมูล" }, { key: "gpValue" as const, label: "GP", value: metric?.hasFilteredSalesData ? `${compact(current.gpValue)} MMK` : "รอข้อมูล" }, { key: "gpPercent" as const, label: "GP%", value: current.gpPercent === null ? "รอข้อมูล" : `${current.gpPercent.toFixed(1)}%` }] : [];
    const waiting = "รอข้อมูล";
    const smartClickRows = metric ? [{ label: "Booking", value: waiting }, { label: "Customer", value: waiting }, { label: "Campaign", value: waiting }, { label: "Competitor", value: waiting }, { label: "Salesman", value: metric.topSalesperson?.trim() || waiting }, { label: "Last Visit", value: waiting }, { label: "Remark", value: waiting }] : [];
    ...
    return <aside aria-label="Right Intelligence Panel" className="hidden min-h-0 flex-col overflow-y-auto rounded-2xl border border-[#E5E7EB] bg-white p-4 text-sm text-[#6B7280] shadow-[0_1px_2px_rgba(31,41,55,0.04)] xl:flex"><div className="sticky top-0 z-10 border-b border-[#EEF0F3] bg-white pb-3">{metric ? <div className="flex items-start justify-between gap-3"><div className="min-w-0"><h2 className="text-lg font-bold tracking-[-0.02em] text-[#1F2937]">{metric.township}</h2><p className="mt-0.5 truncate text-xs font-semibold text-[#6B7280]">{metric.stateRegion} · {metric.responsibleShowroom ?? "ยังไม่ได้กำหนด Showroom รับผิดชอบ"}</p><p className="mt-2 truncate text-[11px] font-semibold text-[#9CA3AF]">{filterContext.year} · {filterContext.month} · {filterContext.product}</p></div><button type="button" onClick={onClose} aria-label="Close Township panel" className="grid size-8 shrink-0 place-items-center rounded-lg border border-[#E5E7EB] text-[#6B7280] hover:bg-[#F9FAFB]"><X size={16}/></button></div> : <><p className="text-xs font-bold uppercase tracking-[0.12em] text-[#9CA3AF]">Township Executive Intelligence</p><h2 className="mt-1 text-base font-bold text-[#1F2937]">เลือก Township บนแผนที่</h2></>}</div>{!metric ? <p className="mt-3 rounded-xl bg-[#FAFBFC] p-3 text-xs leading-5">เลือก Township บนแผนที่เพื่อดูรายละเอียด</p> : <div className="space-y-4 pt-3"><section><h3 className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#9CA3AF]">Current Performance</h3>{!metric.hasFilteredSalesData && <p className="mt-2 rounded-xl border border-dashed border-[#D1D5DB] bg-white p-4 text-center text-sm font-semibold text-[#6B7280]">ไม่พบข้อมูลตามตัวกรองที่เลือก</p>}<div className="mt-2 grid grid-cols-2 gap-2">{primaryRows.map((row) => <div key={row.key} className={cn("rounded-lg border px-3 py-2", activeMetric === row.key ? "border-[#FFB46E] bg-[#FFF7EF]" : "border-[#EEF0F3] bg-white")}><b className={cn("block text-[15px] leading-5", activeMetric === row.key ? "text-[#E86F00]" : "text-[#1F2937]")}>{row.value}</b><span className="mt-0.5 block text-[10px] font-semibold uppercase tracking-wide text-[#8A8E96]">{row.label === "Unit" ? "Sales Unit" : row.label === "Value" ? "Sales Value" : row.label}</span></div>)}</div></section><section><h3 className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#9CA3AF]">Year-over-Year Performance</h3>{yoyMessage ? <p className="mt-2 rounded-lg bg-[#FAFBFC] p-3 text-xs font-semibold">{yoyMessage}</p> : <div className="mt-2 divide-y divide-[#EEF0F3] rounded-lg border border-[#EEF0F3]">{yoyRows.map((row) => <div key={row.key} className="px-3 py-2 text-xs"><div className="flex items-center justify-between gap-2"><span className="font-semibold text-[#4B5563]">{row.label}</span><b className="text-[#1F2937]">{row.current === null ? waiting : row.key === "gpPercent" ? `${row.current.toFixed(1)}%` : row.key === "salesUnit" ? count(row.current) : `${compact(row.current)} MMK`}</b><span className="font-bold text-[#4B5563]">{row.delta === null ? waiting : row.key === "gpPercent" ? `${changeArrow(row.delta)} ${Math.abs(row.delta).toFixed(1)} pts` : `${changeArrow(row.delta)} ${row.percentage === null ? waiting : `${Math.abs(row.percentage).toFixed(1)}%`}`}</span></div>{row.key !== "gpPercent" && row.delta !== null && <p className="mt-0.5 text-[10px] text-[#8A8E96]">เทียบปีก่อน: {row.delta >= 0 ? "+" : ""}{row.key === "salesUnit" ? count(row.delta) : `${compact(row.delta)} MMK`}</p>}</div>)}</div>}</section><section className="border-t border-[#EEF0F3] pt-3"><h3 className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#9CA3AF]">Booking · Customer · Campaign</h3><dl className="mt-2 grid gap-2 text-xs">{smartClickRows.map((row) => <div key={row.label} className="flex items-center justify-between gap-3"><dt className="font-semibold text-[#6B7280]">{row.label}</dt><dd className="text-right font-bold text-[#1F2937]">{row.value}</dd></div>)}</dl></section>{metric.hasFilteredSalesData && <><section><h3 className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#9CA3AF]">Product Mix</h3>{mix.length ? <><p className="mt-2 text-xs"><span className="font-semibold text-[#8A8E96]">Top Product</span><br/><b className="text-[#1F2937]">{leadingMix?.label} · {Math.round(((leadingMix?.unit ?? 0) / metric.salesUnit) * 100)}%</b></p><div className="mt-2 space-y-2">{mix.map((row) => <div key={row.key} className="text-xs"><div className="flex justify-between gap-2"><span className="font-semibold text-[#4B5563]">{row.label}</span><b>{count(row.unit)} · {Math.round((row.unit / metric.salesUnit) * 100)}%</b></div><div className="mt-1 h-1 rounded-full bg-[#F3F4F6]"><div className="h-full rounded-full bg-[#FF8615]" style={{ width: `${(row.unit / maxMix) * 100}%` }}/></div></div>)}</div></> : <p className="mt-2 rounded-lg bg-[#FAFBFC] p-3 text-xs font-semibold">ไม่มีข้อมูลยอดขายแยกตามสินค้า</p>}</section><section><h3 className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#9CA3AF]">Township Benchmark</h3>{benchmark ? <div className="mt-2 rounded-lg border border-[#EEF0F3] p-3 text-xs"><p className="font-semibold text-[#1F2937]">อันดับ #{benchmark.rank} จาก {benchmark.count} Township</p><p className="mt-1 font-bold text-[#E86F00]">{percentile}</p><p className="mt-2">Township Average: <b>{activeMetric === "gpPercent" ? `${benchmark.average.toFixed(1)}%` : activeMetric === "salesUnit" ? count(benchmark.average) : `${compact(benchmark.average)} MMK`}</b></p><p className="mt-1">{changeArrow(benchmark.value - benchmark.average)} {activeMetric === "gpPercent" ? `${Math.abs(benchmark.value - benchmark.average).toFixed(1)} pts` : benchmark.average ? `${Math.abs(((benchmark.value - benchmark.average) / benchmark.average) * 100).toFixed(1)}% vs Township Average` : waiting}</p></div> : <p className="mt-2 rounded-lg bg-[#FAFBFC] p-3 text-xs font-semibold">ไม่มีข้อมูลเปรียบเทียบ Township</p>}</section><section><h3 className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#9CA3AF]">ประเด็นสำคัญ <span className="normal-case">Key Insights</span></h3>{insights.length ? <ul className="mt-2 space-y-2">{insights.map((insight) => <li key={insight.category} className="border-l-2 border-[#FFB46E] pl-2 text-xs"><b className="block text-[#4B5563]">{insight.category}</b><span>{insight.text}</span></li>)}</ul> : <p className="mt-2 rounded-lg bg-[#FAFBFC] p-3 text-xs font-semibold">ไม่มีประเด็นเพิ่มเติมจากข้อมูลที่เลือก</p>}</section></> }</div>}</aside>;
}
...
export function MarketingIntelligencePage() {
    ...
    const mapped = useMemo(() => { const population = new Map<string, number>(); const activities = new Map<string, number>(); const salesUnits = new Map<string, number>(); const salesValues = new Map<string, number>(); const gpValues = new Map<string, number>(); const installedBaseByProduct = new Map<string, TownshipMetric["installedBaseByProduct"]>(); const salesByProduct = new Map<string, TownshipMetric["salesByProduct"]>(); const salespeople = new Map<string, Map<string, number>>(); const lastActivityDates = new Map<string, string>(); const activityTypes = new Map<string, Map<string, number>>(); const source = new Map<string, GeoTownship>(); const productKey = (category: string): keyof TownshipMetric["salesByProduct"] => category === "TT" ? "tractor" : category === "CH" ? "combineHarvester" : category === "EX" ? "excavator" : category === "TP" ? "transplanter" : category === "MAX" ? "drone" : "other"; const blankProducts = (): TownshipMetric["salesByProduct"] => ({ tractor: 0, combineHarvester: 0, excavator: 0, transplanter: 0, drone: 0, other: 0 }); populationSales.forEach((row) => { const resolved = resolveTownship(row.township, row.stateRegion); if (resolved.key) {
        ...
        if (row.salesperson.trim()) { const counts = salespeople.get(resolved.key) ?? new Map<string, number>(); counts.set(row.salesperson, (counts.get(row.salesperson) ?? 0) + 1); salespeople.set(resolved.key, counts); }
