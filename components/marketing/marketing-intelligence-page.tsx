...
function Trend({ rows }: { rows: MarketingRow[] }) { const latest = Math.max(...rows.map((row) => row.month ?? 0), 0); const values = MONTHS.map((_, index) => index + 1 > latest ? null : rows.filter((row) => row.month === index + 1).length); return <PremiumTrendChart labels={MONTHS} unit="Activities" series={[{ id: "activities", label: "Monthly Activities", values, kind: "current" }]} />; }
