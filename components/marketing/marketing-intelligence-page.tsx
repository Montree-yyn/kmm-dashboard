...
function Phase1TownshipPanel({ metric, activeMetric, filterContext }: { metric: TownshipMetric | null; activeMetric: ExecutiveGisFilters["activeMetric"]; filterContext: { year: string; month: string; product: string } }) {
    const rows = metric ? [
        { key: "salesUnit" as const, label: "Unit", value: count(metric.salesUnit) },
        { key: "salesValue" as const, label: "Value", value: `${compact(metric.salesValue)} MMK` },
        { key: "gpValue" as const, label: "GP", value: `${compact(metric.gpValue)} MMK` },
        { key: "gpPercent" as const, label: "GP%", value: metric.gpPercent === null ? "N/A" : `${metric.gpPercent.toFixed(1)}%` },
    ] : [];
    return <aside aria-label="Right Intelligence Panel" className="hidden min-h-0 flex-col overflow-y-auto rounded-2xl border border-[#E5E7EB] bg-white p-4 text-sm text-[#6B7280] shadow-[0_1px_2px_rgba(31,41,55,0.04)] xl:flex"><div className="sticky top-0 bg-white pb-3"><p className="text-xs font-bold uppercase tracking-[0.12em] text-[#9CA3AF]">Township Detail</p>{metric ? <><h2 className="mt-1 text-base font-bold text-[#1F2937]">{metric.township}</h2><p className="mt-0.5 text-xs font-semibold text-[#9CA3AF]">{metric.stateRegion}</p></> : <h2 className="mt-1 text-base font-bold text-[#1F2937]">เลือก Township บนแผนที่</h2>}</div>{!metric ? <p className="mt-2 rounded-xl bg-[#FAFBFC] p-3 text-xs leading-5">เลือก Township บนแผนที่เพื่อดูรายละเอียด</p> : <div className="mt-2 space-y-4"><section className="rounded-xl bg-[#FAFBFC] p-3"><p className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#9CA3AF]">Showroom</p><p className="mt-1 font-semibold text-[#1F2937]">{metric.responsibleShowroom ?? "ยังไม่ได้กำหนด Showroom รับผิดชอบ"}</p></section><section className="grid gap-2 rounded-xl bg-[#FAFBFC] p-3 text-xs"><div className="flex justify-between gap-3"><span>ปี</span><b className="truncate text-[#1F2937]">{filterContext.year}</b></div><div className="flex justify-between gap-3"><span>เดือน</span><b className="truncate text-[#1F2937]">{filterContext.month}</b></div><div className="flex justify-between gap-3"><span>สินค้า</span><b className="truncate text-[#1F2937]">{filterContext.product}</b></div></section>{!metric.hasFilteredSalesData ? <p className="rounded-xl border border-dashed border-[#D1D5DB] bg-white p-4 text-center text-sm font-semibold text-[#6B7280]">ไม่พบข้อมูลตามตัวกรองที่เลือก</p> : <section className="space-y-2">{rows.map((row) => <div key={row.key} className={cn("flex items-center justify-between rounded-xl border px-3 py-2", activeMetric === row.key ? "border-[#FFB46E] bg-[#FFF7EF] text-[#E86F00]" : "border-[#EEF0F3] bg-white text-[#4B5563]")}><span className="font-semibold">{row.label}</span><b className="text-base">{row.value}</b></div>)}</section>}</div>}</aside>;
}
