...
type ComparisonTownshipSummary = { id: CanonicalTownshipId; township: string; stateRegion: string };
function PhaseAComparisonPanel({ selectedTownships, message, onRemove, onClear, onExit }: { selectedTownships: ComparisonTownshipSummary[]; message: string; onRemove: (id: CanonicalTownshipId) => void; onClear: () => void; onExit: () => void }) {
    const selectedCount = selectedTownships.length;
    return <aside aria-label="Right Intelligence Panel" className="hidden min-h-0 flex-col overflow-y-auto rounded-2xl border border-[#E5E7EB] bg-white p-4 text-sm text-[#6B7280] shadow-[0_1px_2px_rgba(31,41,55,0.04)] xl:flex"><div className="sticky top-0 z-10 border-b border-[#EEF0F3] bg-white pb-3"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.12em] text-[#9CA3AF]">Area Comparison</p><h2 className="mt-1 text-base font-bold text-[#1F2937]">เปรียบเทียบพื้นที่</h2>{selectedCount > 0 && <p className="mt-1 text-xs font-bold text-[#E86F00]">{selectedCount} / {MAX_COMPARISON_TOWNSHIPS} Township</p>}</div><button type="button" onClick={onExit} className="rounded-lg border border-[#E5E7EB] px-2.5 py-1.5 text-xs font-bold text-[#6B7280] hover:bg-[#F9FAFB]">ออกจากโหมดเปรียบเทียบ</button></div></div><div className="space-y-3 pt-3">{message && <p role="status" className="rounded-lg border border-[#FED7AA] bg-[#FFF7EF] px-3 py-2 text-xs font-bold text-[#C2410C]">{message}</p>}{selectedCount === 0 ? <div className="rounded-xl bg-[#FAFBFC] p-4 text-xs leading-5"><p className="font-bold text-[#1F2937]">เลือกพื้นที่บนแผนที่เพื่อเริ่มต้น</p><p className="mt-1">คลิก Township เพื่อเพิ่มรายการเปรียบเทียบ</p></div> : <><div className="flex items-center justify-between gap-2"><p className="text-xs font-bold text-[#6B7280]">{selectedCount === 1 ? "เลือกอีก 1 Township เพื่อเริ่มเปรียบเทียบ" : "รายการที่เลือกสำหรับ Phase A"}</p><button type="button" onClick={onClear} className="text-xs font-bold text-[#E86F00] hover:text-[#C2410C]">ล้างทั้งหมด</button></div><div className="space-y-2">{selectedTownships.map((township, index) => <div key={township.id} className="flex min-h-10 items-center gap-2 rounded-lg border border-[#EEF0F3] bg-white px-3 py-2"><span className="grid size-6 shrink-0 place-items-center rounded-full bg-[#E86F00] text-xs font-bold text-white">{index + 1}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-[#1F2937]">{township.township}</p><p className="truncate text-[11px] font-semibold text-[#8A8E96]">{township.stateRegion}</p></div><button type="button" onClick={() => onRemove(township.id)} className="grid size-8 shrink-0 place-items-center rounded-lg border border-[#E5E7EB] text-xs font-bold text-[#6B7280] hover:bg-[#F9FAFB]" aria-label={`Remove ${township.township} from comparison`}>×</button></div>)}</div></>}</div></aside>;
}
export function MarketingIntelligencePage() {
  ...
  const showroomMetrics = useMemo(() => OPERATIONAL_SHOWROOMS.map((showroom) => { const sales = periodSales.filter((row) => operationalShowroomForBranch(row.branch)?.code === showroom.code); const activity = marketing.filter((row) => operationalShowroomForBranch(row.branch)?.code === showroom.code); const bookings = (data?.booking ?? []).filter((row) => currentMatch(row, filters) && operationalShowroomForBranch(row.branch)?.code === showroom.code && row.status !== "Cancelled"); const unit = sales.filter((row) => UNIT_PRODUCTS.includes(productGroup(row.productType))); const value = sales.filter((row) => VALUE_PRODUCTS.includes(productGroup(row.productType))); const cost = sum(activity, (row) => row.expense); const revenue = sum(value, (row) => row.finalReceived); const months = MONTHS.map((label, index) => ({ label, cost: sum(activity.filter((row) => row.month === index + 1), (row) => row.expense), unit: unit.filter((row) => row.month === index + 1).length })); return { showroom, activities: activity.length, unit: unit.length, value: revenue, gp: sum(value, (row) => row.gp1), cost, booking: bookings.length, roi: cost ? revenue / cost : null, topProduct: <ShowroomChart months={months}/>, topSalesperson: "", months }; }), [periodSales, marketing, data, filters]);
    ...
    const selectedComparisonTownships = selectedComparisonTownshipIds.map((id) => {
        ...
        return township ? { id, township: metric?.township ?? township.township, stateRegion: metric?.stateRegion ?? township.stateRegion } : null;
