...
function HorizontalBars({ data, emptyText = "No source data available" }: { data: { label: string; value: number | null }[]; emptyText?: string }) {
  ...
  return <div className="space-y-4">{visible.map((item) => <div key={item.label} className="grid grid-cols-[minmax(78px,120px)_minmax(0,1fr)_auto] items-center gap-3 text-sm"><span className="truncate font-semibold text-[#4B5563]" title={item.label}>{item.label}</span><Progress value={((item.value ?? 0) / max) * 100} className="h-2.5" indicatorClassName="bg-[var(--chart-current)]" /><span className="min-w-10 text-right text-xs font-bold text-[#4B5563]">{formatCompact(item.value ?? 0)}</span></div>)}</div>;
}

function TrendChart({ data, mode }: { data: TrendPoint[]; mode: "activities" | "cost-sales" }) { return <PremiumTrendChart title={mode === "activities" ? "Marketing Activities Trend" : "Marketing Cost vs Sales"} subtitle={mode === "activities" ? "Monthly completed marketing activities" : "Sales attribution is unavailable in the current marketing extract"} labels={data.map((point) => point.label)} unit={mode === "activities" ? "Activities" : "MMK"} formatValue={formatCompact} series={mode === "activities" ? [{ id: "activities", label: "Monthly Activities", values: data.map((point) => point.activities), kind: "current" }] : [{ id: "cost", label: "Marketing Cost", values: data.map((point) => point.cost), kind: "current" }, { id: "sales", label: "Sales", values: data.map((point) => point.sales), kind: "previous" }]} />; }
