...
const MYANMAR_BOUNDS: [[number, number], [number, number]] = [[90.6, 7.6], [103, 30.1]];
...
function fitMyanmar(map: MapLibreMap, duration = 0) {
  map.fitBounds(MYANMAR_BOUNDS, { duration, padding: 40, maxZoom: 4 });
}
...
export function MyanmarMarketingMap({ visibleShowroomIds, townshipMetrics = {}, productLabel = "All", mode = "population", className }: MyanmarMarketingMapProps) {
  const containerRef = useRef<HTMLDivElement | null>(null); const mapRef = useRef<MapLibreMap | null>(null); const townshipsRef = useRef<GeoFeature[]>([]); const markersRef = useRef(new Map<string, Marker>()); const metricsRef = useRef(townshipMetrics); const productRef = useRef(productLabel); const visibleIdsRef = useRef(visibleShowroomIds); const [status, setStatus] = useState<"loading" | "ready" | "error">("loading");
