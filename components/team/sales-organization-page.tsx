...
export function SalesOrganizationPage() {
  ...
  const selectedRows = selectedPerson ? activeSales.filter((row) => resolveCommissionIdentity(row, data?.employees ?? [], data?.salespersonIdentityAliases ?? [])?.key === selectedPerson.id) : [];
