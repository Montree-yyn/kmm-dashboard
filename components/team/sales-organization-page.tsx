...
function SalespersonCard({ person, currency, onSelect }: { person: Person; currency: string; onSelect: (person: Person) => void }) {
  return <article role="button" tabIndex={0} aria-label={`View ${person.name} details`} onClick={() => onSelect(person)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onSelect(person); } }} className="group rounded-[var(--radius-card)] border border-[var(--border-default)] bg-[var(--surface-default)] p-5 shadow-[var(--shadow-card)] transition-[border-color,box-shadow,transform] duration-200 hover:-translate-y-0.5 hover:border-[var(--text-disabled)] hover:shadow-[var(--shadow-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)]"><div className="flex items-start justify-between gap-3"><span className="grid size-8 place-items-center rounded-full bg-[var(--brand-500)] text-sm font-bold text-[var(--text-primary)]">{person.rank}</span><Avatar name={person.name} large /></div><div className="mt-4 min-w-0 text-center"><p className="truncate text-base font-semibold text-[var(--text-primary)]" title={person.name}>{person.name}</p><p className="mt-1 truncate text-sm text-[var(--text-secondary)]">{person.branch}</p>{person.position && <p className="mt-1 truncate text-xs text-[var(--text-tertiary)]">{person.position}</p>}</div><div className="mt-5 grid grid-cols-2 gap-3 border-t border-[var(--divider)] pt-4"><Metric label="Sales Unit" value={formatNumber(person.salesUnit)} /><Metric label="Sales Value" value={formatCurrency(person.salesValue, currency)} /><Metric label="GP Value" value={formatCurrency(person.gp, currency)} /><Metric label="GP %" value={<RankingMetricCell person={person} metric="gpPercent" />} /><Metric label="Commission" value={formatCurrency(person.commission, currency)} /></div><Button type="button" variant="outline" className="mt-5 w-full" onClick={(event) => { event.stopPropagation(); onSelect(person); }}>View Detail <span aria-hidden="true">→</span></Button></article>;
}
