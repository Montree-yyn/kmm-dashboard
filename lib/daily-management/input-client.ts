...
export async function loadDailyManagementInput(mode: "draft" | "published", options: { date?: string; branch?: string } = {}) {
...
export async function persistDailyManagementInput(snapshot: DailyManagementInputSnapshot, mode: "draft" | "publish") {
  const response = await fetch("/api/daily-management/input", {
    method: "PUT",
