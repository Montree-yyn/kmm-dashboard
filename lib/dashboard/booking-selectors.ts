...
// `asOf` is intentionally required: booking age must always be measured
// against a caller-supplied date (the server's company-timezone "business
// today"), never a hardcoded default that silently freezes the metric.
export function bookingAge(row: BookingRow, asOf: Date) { const date = new Date(`${row.date}T00:00:00`); const days = Math.floor((asOf.getTime() - date.getTime()) / 86_400_000); return Number.isFinite(days) ? Math.max(0, days) : 0; }
export function getAverageBookingAge<T extends BookingRow>(rows: T[], filters: BookingFilters = {}, asOf: Date) { const openRows = getOpenBookingUnitRows(rows, filters); return openRows.length ? openRows.reduce((total, row) => total + bookingAge(row, asOf), 0) / openRows.length : null; }
...
export function getBookingByBranch<T extends BookingRow>(rows: T[], filters: BookingFilters = {}) { return [...new Set(rows.map((row) => row.branch).filter(Boolean))].sort().map((branch) => { const branchRows = getOpenBookingRows(rows, { ...filters, branch: [branch] }); return { branch, unit: branchRows.length, value: branchRows.reduce((total, row) => total + numberOrZero(row.price), 0), deposit: branchRows.reduce((total, row) => total + numberOrZero(row.deposit), 0) }; }); }
