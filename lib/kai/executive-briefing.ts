...
type Snapshot = { period:{start:string;end:string;scopeLabel:string}; sales:{units:number;value:number;gp:number;gpPercent:number|null}; booking:{units:number;value:number}; stock:{units:number;value:number;snapshotDate:string|null}; target:{target:number}|null; progress:{achievementPercent:number;gap:number}|null };
...
export function composeExecutiveBriefing(snapshot: Snapshot, alerts: ExecutiveAlert[], recommendations: ExecutiveRecommendation[], mode: "daily"|"weekly"|"monthly", isMtd: boolean) {
  ...
  if (snapshot.target && snapshot.progress) lines.push(`• ${isMtd ? "Monthly Target Context" : "Target"}: ${number(snapshot.target.target)} คัน · ${isMtd ? "MTD " : ""}Achievement ${number(snapshot.progress.achievementPercent)}%${isMtd ? " · เดือนยังไม่สิ้นสุด จึงยังไม่ประเมินว่า Target ทั้งเดือนล้มเหลว" : ` · Gap ${number(snapshot.progress.gap)}`}`);
